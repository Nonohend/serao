...
-- Ajoute :
--   P1  Champs boutique sur profiles (shop_name, shop_description, shop_banner_url)
--   P2  Champs moyennes de notes sur profiles (rating_avg, rating_count)
--   P3  Table user_ratings + RLS
--   P4  Trigger de recalcul automatique des moyennes
--   P5  RPC get_public_profile() — lecture sécurisée
--   P6  RPC submit_user_rating()
-- Idempotent.
-- =============================================

-- ---------------------------------------------------------------
-- P1 & P2 — Champs boutique et notes sur profiles
-- ---------------------------------------------------------------
...
CREATE POLICY "ratings_read" ON public.user_ratings
  FOR SELECT USING (true);

-- Chacun peut noter (pas soi-même, pas deux fois le même)
...
-- ---------------------------------------------------------------
-- P4 — Trigger de recalcul des moyennes
-- ---------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.recalc_user_rating()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  target_id uuid := COALESCE(NEW.evalue_id, OLD.evalue_id);
BEGIN
...
-- ---------------------------------------------------------------
-- P5 — get_public_profile() : lecture sécurisée d'un profil
-- ---------------------------------------------------------------
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.submit_user_rating(uuid, integer, text, bigint, text) TO authenticated;
